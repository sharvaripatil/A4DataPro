...
public class BamPriceGridParser {
	...
	public List<PriceGrid> getBasePriceGrids(String listOfNetCost,
	...
	public List<PriceConfiguration> getConfigurations(String criterias,String value,String another) {
		List<PriceConfiguration> listOfpriceConfiguration = new ArrayList<PriceConfiguration>();
		PriceConfiguration configs = null;
		try{//PROP:optionName
			if(criterias.contains("PROP") || criterias.contains("IMOP")){
				String[] temp = criterias.split(":");
				String criteriaValue = LookupData.getCriteriaValue(temp[0]);
				String optionName = temp[1];
				List<String> optionVals = Arrays.asList(value.split(","));
				for (String optVal : optionVals) {
					configs = new PriceConfiguration();
					configs.setCriteria(criteriaValue);
				...
				configs = new PriceConfiguration();
				String criteriaValue = LookupData.getCriteriaValue(criterias);
				configs.setCriteria(criteriaValue);
				configs.setValue(Arrays.asList((Object) value));
			}
			...
			_LOGGER.error("Error while processing PriceGrid configuration: "+e.getMessage());
		}
