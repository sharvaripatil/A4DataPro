...
public class GoldstarCanadaColorParser {
	...
	public List<Color> getColorCriteria1(String colorValue) {
		...
		for (String outervalue : colorsList) {
			...
			for (int i=0;i<1;i++) {
					...
					String color2 = GoldstartCanadaApplicationConstatnt.COLOR_MAP.get(colorArr1[1]);
				...
				String color1 = GoldstartCanadaApplicationConstatnt.COLOR_MAP.get(colorArr1[0]);
				...
				String outerColor = GoldstartCanadaApplicationConstatnt.COLOR_MAP.get(outervalue);
		...
		for (String colorVal : colorArr) {//avoid duplicate values
 		...
 		for (String colorName : colorsSet) {
 			...
 			if(colorName.contains("/")){
 				...
 				String colorGroup = GoldstartCanadaApplicationConstatnt.getColorGroup(colorName);
	...
	private Color getColorCombo(String comboVal){
        ...
        colorObj.setName(
        		GoldstartCanadaApplicationConstatnt.getColorGroup(comboColors[ApplicationConstants.CONST_NUMBER_ZERO].toLowerCase()));
        comboObj1.setName(
        		GoldstartCanadaApplicationConstatnt.getColorGroup(comboColors[ApplicationConstants.CONST_INT_VALUE_ONE].toLowerCase()));
        ...
        if(comboColors.length == 3){
              comboObj2.setName(
            		  GoldstartCanadaApplicationConstatnt.getColorGroup(comboColors[ApplicationConstants.CONST_INT_VALUE_TWO].toLowerCase()));
